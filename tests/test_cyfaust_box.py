...
def test_box_vslider():
    ...
    with box_context():
        b = box_mul(
            box_vslider(
                "h:Oscillator/freq", box_real(440), box_real(50), box_real(1000), box_real(0.1)
            ),
            box_vslider("h:Oscillator/gain", box_real(0), box_real(0), box_real(1), box_real(0.01)),
        )
        assert b.is_valid()
        b.print()
...
def test_box_fconst():
    ...
    def SR():
        return box_min(
            box_real(192000.0),
            box_max(box_real(1.0), box_fconst(SType.kSInt, "fSampleFreq", "<math.h>")),
        )
...
def test_box_soundfile():
    ...
    with box_context():
        b = box_soundfile("sound[url:{'tango.wav'}]", box_int(2), box_int(0), box_int(0))
        assert b.is_valid()
        b.print()
