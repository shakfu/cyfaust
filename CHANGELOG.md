...
- Extended the runtime UI parameter API (`params()`, `get_param`, `set_param`) to the LLVM JIT backend's `LlvmDsp`, so the static build made with `-DLLVM=ON` has the same parameter access as the interpreter. `LlvmDsp` was previously missing the API entirely -- the interpreter additions above only reached the static interpreter classes through `scripts/generate_static.py`, whereas `backend_llvm.pxi` is hand-maintained and is not regenerated. The port reuses the module-level `Param` namedtuple and widget-kind tables emitted into `cyfaust.pyx` by the amalgamation, so behaviour is identical across backends. `tests/test_cyfaust_params.py` now parametrizes every case over both backends, skipping the LLVM variants when that backend is not compiled in
...
- Fixed `scripts/generate_static.py` emitting CRLF line endings on Windows: it wrote `src/static/cyfaust/cyfaust.pyx` and the synced `.pxd` files via `Path.write_text()`, which translates `\n` to the platform line ending, so regenerating on Windows produced CRLF files that conflict with the repo's `.gitattributes` (`eol=lf`) and showed up as phantom diffs. Both writes now go through `Path.write_bytes(text.encode("utf-8"))`, which performs no newline translation (LF on every platform) and, unlike the `write_text(newline=...)` keyword (Python 3.10+), also runs under the manylinux build container's Python 3.6 system interpreter -- an earlier `newline=""` fix broke the Linux wheel job with `TypeError: write_text() got an unexpected keyword argument 'newline'`
...
- Built against Faust 2.85.9 (both the interpreter `libfaust` and the LLVM JIT `libfaustwithllvm` provisioned by `make faust`/`make faustwithllvm`)
- Extended `scripts/verify_build_sync.py` (the `verify-sync` make target) with a runtime DSP class parity check: it now asserts that the interpreter's `InterpreterDsp` and the LLVM JIT's `LlvmDsp` expose the same public method set. Previously the guard only compared the shared `.pxd` files, so a method added to `InterpreterDsp` (amalgamated into the static source by `generate_static.py`) could silently miss the hand-maintained `LlvmDsp` in `backend_llvm.pxi` -- which is exactly how the runtime param API first shipped without an LLVM counterpart. Intentional, currently-unportable gaps (`control`, `frame`, `compute_timestamped`, none of which the `llvm_dsp` binding declares) are recorded in an allowlist with reasons; any other divergence, and any stale allowlist entry, fails the check. Adds `tests/test_verify_build_sync.py`
